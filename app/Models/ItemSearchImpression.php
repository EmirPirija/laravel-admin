...
{
    protected $table = 'item_search_impressions';
    ...
    private static function clickColumn(): string
    {
        static $column = null;
        ...
        $column = Schema::hasColumn('item_search_impressions', 'clicked') ? 'clicked' : 'was_clicked';
        return $column;
    }
    ...
    public static function getSearchStats(int $itemId, int $days = 30): array
    {
        ...
        $clickColumn = self::clickColumn();

        $stats = self::where('item_id', $itemId)
            ->where('created_at', '>=', $startDate)
            ->selectRaw("
                COUNT(*) as impressions,
                SUM(CASE WHEN {$clickColumn} = 1 THEN 1 ELSE 0 END) as clicks,
        ...
        $topQueries = self::where('item_id', $itemId)
            ...
            ->where('search_query', '!=', '')
            ->selectRaw("search_query, COUNT(*) as count, SUM(CASE WHEN {$clickColumn} = 1 THEN 1 ELSE 0 END) as clicks")
